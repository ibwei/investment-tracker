...
- Assets：系统自动读取 CEX / 链上资产来源，并允许用户手动维护无法自动同步的资产，用于总资产、币种分布、平台分布和资产历史趋势
...
- 聚合用户多个 CEX、链上地址和手动录入资产，形成总资产视图
- 展示资产按币种、平台、来源类型的分布
- 展示总资产和主要资产的历史变化趋势
- 降低用户手动维护资产余额的成本
- 允许用户补充无法通过 API 或链上 provider 自动读取的资产，例如银行现金、股票基金、线下资产或其他自定义资产
...
用户可以手动添加无法自动追踪或暂不适合接入第三方 API 的资产。这类资产不需要外部同步，但必须参与总资产、资产分布和资产快照采集。

典型资产：

- 银行存款、现金和备用金
- 美股、港股、A 股、基金、ETF
- 未接入交易所或钱包 provider 的资产
- 线下资产或其他自定义资产

输入字段：

- 名称，例如 `招商银行活期`、`VOO ETF`、`Private deal`
- 类型，例如 `Cash`、`Stock`、`Fund`、`Token`、`Real Estate`、`Other`
- 金额，用于记录原始数量或本位金额，例如 `10000`、`25.5`
- 美元本位数量，即 `valueUsd`，用于资产聚合和快照计算
- 备注，可选，用于记录来源说明、估值口径或更新提示
...
3. 金额和美元本位数量必须为非负数
4. 保存手动资产记录并标记更新时间
5. 手动资产写入或更新后重新捕获当前用户资产快照
6. 返回最新 Assets dashboard snapshot

手动资产规则：

- 手动资产不调用 CEX 或链上 adapter
- 手动资产不参与 Cron 同步
- 手动资产需要支持编辑和删除
- 删除手动资产后需要重新捕获当前用户资产快照
- 预览模式展示 mock 手动资产，但不能保存真实写入
- 资产聚合统一使用美元本位数量 `valueUsd`
...
- 按类别分布：Spot、Earn、DeFi、Cash、Stock、Fund、Real Estate、Other
- 资产来源列表
- 手动资产列表
- 资产明细列表
...
- Cron 同步结束后生成的资产快照必须包含当前用户已启用的手动资产
...
- 手动资产表单需要明确提示“美元本位数量会计入总资产”
- 手动资产的备注只在用户自己的页面和 API 响应中展示，不进入公开日志
- 页面必须按需加载数据。首屏只请求总资产摘要和少量概览，不一次性拉取余额明细、DeFi 仓位、完整趋势和同步日志
...
- 按币种
- 按来源
- 按来源类型：CEX / ONCHAIN / MANUAL
- 按资产类别：Spot / Earn / DeFi / Cash / Stock / Fund / Real Estate / Other
...
手动资产不显示在自动资产来源列表中，避免用户误以为它会被 Cron 或第三方 adapter 更新。
...
手动资产需要支持快速编辑。保存后应刷新总览、分布和趋势入口数据，并提示用户已重新采集当前资产快照。
...
Assets 页面数据量可能很大，UI 折叠只是视觉层优化，接口层也必须按需加载，避免打开 `/assets` 时一次性读取所有表和外部相关明细。
...
- 打开 `Trend` tab 时请求趋势数据
- 打开 `Sources` tab 时请求 source 列表摘要
- 展开某个 source 时请求该 source 的余额、仓位和同步状态详情
- 打开 `Manual` tab 时请求手动资产列表
- 打开 `Balances` tab 时请求分页资产明细
- 打开 `DeFi` tab 时请求分页 DeFi 仓位
- 打开 `Health` tab 时请求失败来源和同步日志摘要

前端状态建议：

- 每个 tab 独立 loading / error / empty 状态
- 已加载 tab 在本次页面会话内缓存，手动刷新或同步成功后再失效
- 首屏接口失败时展示页面级错误；详情接口失败时只影响对应 tab 或展开项
- 同步或编辑手动资产成功后，至少刷新 summary；当前已打开的详情 tab 可按需刷新
...
用户手动维护的资产当前态。它不依赖 `asset_sources`，也不参与外部同步，但必须参与总资产、分布和快照采集。
...
- `name`: 用户可识别的资产名称
- `type`: `CASH`、`STOCK`、`FUND`、`TOKEN`、`REAL_ESTATE`、`OTHER` 等
- `amount`: 用户录入的原始金额或数量
- `value_usd`: 资产聚合使用的美元本位数量
- `note`: 用户备注，不进入日志
- `is_deleted` / `deleted_at`: 建议软删除，便于保留历史操作语义；快照只采集未删除记录
...
- `manual-assets.ts`: 手动资产 CRUD 和软删除
...
- `GET /api/assets`
  - 可作为兼容入口或聚合入口，但不应默认返回全部详情
  - 建议重定向到或内部复用 `/api/assets/summary`

### 12.2 Asset Sources API

- `GET /api/assets/sources`
  - 按需列出当前用户资产来源摘要
  - 支持 `status`、`type`、`limit`、`offset`
...
- `GET /api/assets/sources/:id`
  - 按需读取单个 source 详情
  - 包含 source 基础信息、同步状态摘要和该 source 的轻量统计
...
- `GET /api/assets/snapshots`
  - 按需查询当前用户资产快照趋势
  - 支持 `days`、`startDate`、`endDate`、`granularity`
  - 默认 `days=30`，不默认返回全量历史
...
- `PATCH /api/assets/manual/:id`
  - 更新当前用户手动资产的名称、类型、金额、美元本位数量或备注
  - 保存后捕获当前用户资产快照
- `DELETE /api/assets/manual/:id`
  - 软删除当前用户手动资产
  - 删除后捕获当前用户资产快照

手动资产 API 不需要 `sync` 操作。所有写接口必须调用 `requireSession()` 并按 `userId + id` 定位记录。

### 12.6 Asset Balances API

- `GET /api/assets/balances`
  - 按需分页读取当前用户资产明细
  - 支持 `limit`、`offset`、`sourceId`、`sourceType`、`category`、`q`、`sort`
  - 默认按 `valueUsd desc` 返回 Top 20
  - 不应在首屏自动请求

### 12.7 Asset Positions API

- `GET /api/assets/positions`
  - 按需分页读取当前用户 DeFi / 结构化仓位
  - 支持 `limit`、`offset`、`sourceId`、`chain`、`protocol`、`positionType`、`sort`
  - 默认按 `netValueUsd desc` 返回 Top 20
  - 不应在首屏自动请求

### 12.8 Asset Health API

- `GET /api/assets/health`
  - 按需读取数据健康摘要、失败来源和最近同步日志摘要
...
10. 合并当前用户未删除的手动资产
11. upsert 今日 `asset_snapshots`
12. 返回最新 `/api/assets` snapshot
...
5. 捕获用户资产快照
6. 返回最新 `/api/assets/summary` snapshot
...
5. 每个受影响用户同步结束后捕获资产快照，快照包含该用户未删除的手动资产
6. 写入 `scheduled_job_logs`

### 13.5 首屏加载流程

1. 页面进入 `/assets`
2. 前端只请求 `GET /api/assets/summary`
3. 渲染总资产、变化、最近同步、来源数量、手动资产数量、失败数量和 Top 分布
4. 不请求 balances、positions、manual、snapshots、health 等详情接口
5. 用户点击某个 tab 或展开某个 source 后，再请求对应详情接口
6. 已请求数据在页面会话内缓存；同步、添加来源、编辑手动资产或删除手动资产后，失效 summary 和当前已打开详情
...
- 用户打开 `Balances` tab
  1. 请求 `GET /api/assets/balances?limit=20&offset=0&sort=valueUsd.desc`
  2. 展示 Top 20 资产明细
  3. 用户点击 `Show more` 后继续分页请求

- 用户展开某个 source
  1. 请求 `GET /api/assets/sources/:id`
  2. 需要余额时请求 `GET /api/assets/sources/:id/balances`
  3. 需要仓位时请求 `GET /api/assets/sources/:id/positions`

- 用户打开 `Trend` tab
  1. 请求 `GET /api/assets/snapshots?days=30`
  2. 切换到 90 天时再请求 `GET /api/assets/snapshots?days=90`

按需详情接口必须分页或限制范围，避免返回无限列表。
...
- 手动资产备注不进入 sync log、scheduled job log 或服务端错误日志
- 手动资产删除建议使用软删除，API 响应和快照采集默认过滤 `is_deleted = false`
...
| Assets | 资产聚合、余额、分布、趋势 | CEX API / 链上地址 / 手动资产 | CEX 和链上周期性同步；手动资产按用户编辑采集 |
...
- 页面兼容桌面和移动端
- 首屏只能请求 `/api/assets/summary` 这类轻量接口，避免一次性拉取完整资产详情
- 详情数据必须按 tab、展开项或分页按需请求
- 长列表接口必须支持 `limit` 和 `offset`，默认限制为 20 条
- 趋势接口必须默认限制时间范围，例如 30 天，不默认返回全部历史
- source 详情、余额、仓位、日志需要拆成独立接口，避免一个 source 展开时拉取不需要的数据
- 首屏 summary 建议控制在小 payload，原则上不超过几十 KB
- 同步任务要可重复执行，不因单个 source 失败中断全局任务
- 单用户资产来源数量建议 MVP 限制为 10 个
- 单用户手动资产数量建议 MVP 限制为 50 个
- 单次手动同步需要节流，避免误触发外部 API rate limit
- 手动资产新增、编辑、删除后应立即重新采集当前资产快照
- 数据刷新后 UI 需要明确显示最新同步时间
- 同步成功后应优先刷新 summary；已打开的详情区再按需刷新
...
- 资产总览、分布、明细、手动资产列表、DeFi 仓位、快照趋势
- 手动同步
- 每 4 小时 Cron 同步
- API Key 加密存储
- sync logs 后端记录
