...
function buildPortfolioSnapshotPayload(records, capturedAt) {
  const normalizedRecords = records.map((record) =>
    normalizeInvestmentRecord(record, capturedAt)
  );
...
export async function captureUserSnapshots(userId, capturedAt = new Date()) {
...
export async function captureSnapshotsForRemoteUsers(capturedAt = new Date()) {
  ...
  for (const user of users) {
    const result = await captureUserSnapshots(user.id, capturedAt);
    processedUsers += 1;
