...
export function normalizeInvestmentRecord(record, referenceDate = new Date()) {
  const normalized = {
    id: record.id,
  ...
  return {
    ...normalized,
    metrics: calculateInvestmentMetrics(normalized, referenceDate)
  };
}

export function buildDashboardSnapshot(records, referenceDate = new Date()) {
  const normalized = records
    .filter((record) => !record.isDeleted && !record.is_deleted)
    .map((record) => normalizeInvestmentRecord(record, referenceDate));
