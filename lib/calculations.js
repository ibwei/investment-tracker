...
export function toDateTimeValue(value) {
  ...
  return [
    ...
    String(parsed.getMonth() + 1).padStart(2, "0"),
    String(parsed.getDate()).padStart(2, "0")
  ].join("-") + ` ${String(parsed.getHours()).padStart(2, "0")}:${String(
    parsed.getMinutes()
  ).padStart(2, "0")}:${String(parsed.getSeconds()).padStart(2, "0")}`;
}
...
function calculatePlannedDays(record) {
  const startDate = parseDateInput(record.startTime);
...
export function calculateInvestmentMetrics(record, referenceDate = new Date()) {
  ...
  const plannedDays = calculatePlannedDays(record);
