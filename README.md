...
- 快照系统：手动捕获当前用户快照；Cron 每小时为远程用户捕获快照
- 资产快照：资产同步或手动资产变更后捕获资产净值快照；Cron 每小时同步资产来源
...
- `0 * * * *`：转发到 `/api/cron/snapshots`，每小时采集收益/组合快照
- `5 * * * *`：转发到 `/api/cron/assets/sync`，每小时第 5 分钟同步资产
...
Cron 时间按 UTC 解释。快照每个整点采集，资产在每小时第 5 分钟同步；自动结束到期投资与到期提醒保持原计划。
